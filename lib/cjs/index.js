...
class WorkerClient extends Client {
    ...
    dispose() {
        this._port.postMessage({ type: worker_js_1.EXIT_TYPE });
        super.dispose();
    }
    ...
    _recv(msg) {
        if (this.status === WORKER_DISPOSED)
            return;
        switch (msg.type) {
            case worker_js_1.READY_TYPE:
                this._ready();
                break;
            case worker_js_1.TASK_COMPLETION_TYPE:
                this._complete(msg);
                break;
            default:
    ...
    _onAddTask(msg) {
        this._port.postMessage({ type: worker_js_1.ADD_TASK_TYPE, ...msg });
    }
}
class MainThreadClient extends Client {
    constructor(pool, options) {
        ...
        this._next = this;
        this._prev = this;
        Promise.resolve().then(() => __importStar(require(pool._entrypointModule))).then(async (mod) => {
...
class WorkerPoolImpl {
    ...
    _launchWorker() {
        ...
        const newClient = new WorkerClient(this, new worker_threads_1.Worker(worker_location_js_1.default, { workerData: data }), { maxConcurency: this._options.workerConcurency }, this._numWorkers);
        this._numWorkers += 1;
